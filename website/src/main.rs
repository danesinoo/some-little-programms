...
fn main() -> Result<(), std::io::Error> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            "-d" => {
                vars.insert("DESTINATION", args_iter.next().unwrap().to_string());
            }
            "-t" => {
                vars.insert("TARGET", args_iter.next().unwrap().to_string());
            }
            _ => {
                // help
...
fn get_folders(path: &PathBuf) -> Vec<PathBuf> {
    ...
    let folders_to_remove = ["layout", "assets", "target", "src", DESTINATION];
    folders.retain(|x| !folders_to_remove.contains(&x.to_str().unwrap()));
